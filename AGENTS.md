...
- **Write each document in its own register.** Episode fiction leaks into prose as readily as into code, and the leak is harder to see because the sentence still reads well. Three registers apply:
  - **Game level.** The bed is the first campaign's premise; "let me remain in bed" is that campaign's demand, and the game's demand is whatever refusal a campaign chooses. `docs/game-concept.md`, `docs/art-direction.md`, `README.md`, `PROJECT_CHARTER.md` and the public site say bed, duvet and Management freely.
  ...
  Naming a concrete example to explain a general mechanism is good writing, not a leak — the test is whether the *definition* depends on the example. Documents may quote a campaign's demand, and may say "not a bed" to state the rule itself. This is review discipline rather than a `check:policy` rule: a term match flags "embedded", "alarming" and "Chairman of the Alarm" long before it catches anything real.
...
- Build the rhythmic and musical foundation from the apparatus of the setting being satirised — stamps, typewriters, pneumatic tubes, punch clocks, telephones, machine hum — rather than from conventional instruments. The machinery enforcing the working day supplies the beat the player resists, so the score is part of the argument rather than decoration over it. Each episode takes its instrumentation from its own setting. Conventional melodic instrumentation is a deliberate exception requiring justification, not the default.
- The semantic audio roles in `docs/episode-grammar-reference.md` are the authoritative list of what must sound. Every role is required, so a moment meant to pass quietly is an authored quiet cue rather than an omission. Adding a role is an engine change; deciding what a role sounds like is content, and each episode draws that from its own setting.
