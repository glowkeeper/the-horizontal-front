...
The compiler also produces a non-scoring guide timeline containing authored actions, rests and one opening count-in. Later phase lead-ins become rests rather than repeated READY instructions. READY and REST are engine-level pauses: pressure integration and dramatic-curve progress exclude those intervals. The compiler ends each pause at the opening edge of the following cue's timing window, keeping instruction, judgement and pressure semantics on the same boundary. For a left or right action, the Phaser adapter puts the timing target directly around the required control: a contracting ring anticipates the authored action time, while the control switches to an explicit `HIT NOW` state throughout the valid timing window. The centre is reserved for actions without a control target, such as REST and the opening READY. Phrase previews, procedural audio and optional haptics can consume the same timeline without recreating rhythm interpretation or becoming required for comprehension.
...
Misses apply both a momentum loss and the phase's authored safety penalty. Successful recovery is deliberately weak at zero momentum and strengthens as consecutive accurate actions rebuild rhythm. This lets opening episodes make individual mistakes inexpensive without allowing alternating correct and incorrect one-key input to recover indefinitely. The phase still owns the recovery, momentum and penalty scalars as data. Rest events compile to intentional gaps rather than scored runtime actions; pressure continues through them.
